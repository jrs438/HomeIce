...
- add_ride_rule is ONLY for recording who drives a recurring leg (it requires kind, from, and to). Do not use it just because the input mentions a day of week or "weekly" — that alone does not imply a ride. If the input describes a recurring class/practice/commitment with no driving or transportation mentioned, use add_event for its next upcoming occurrence instead (there is no recurring-event action, so only that one occurrence gets added), and set clarification_needed to a short note telling the parent it repeats weekly and they can ask again to also set up a ride for it. Only emit add_ride_rule when the input actually names or implies transportation (a driver, "pickup"/"drop-off", "needs a ride", "get him there/back", etc.) — and if it does but leaves from/to unstated, default from/to to "Home" and the location mentioned, rather than leaving them blank.
